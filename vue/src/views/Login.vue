<template>
  <div class="login-container">
    <div class="login-box">
      <div style="padding: 50px 30px;background-color: white; margin-bottom: 60px; margin-top:50px; border-radius: 5px; box-shadow: 0 0 10px rgba(0,0,0,0.2)">
  ...
  formRef.value.validate((valid)=>{
    if(valid){
      if(data.form.role==='ADMIN'){
        // 管理员用户
        request.post('/adminLogin',data.form).then(res=>{
        ...
        //学生用户和代取员用户
        request.post('/stuLogin',data.form).then(res=>{
          if(res.code==='200'){//登入成功
            //存储data
            localStorage.setItem('user',JSON.stringify(res.data))
            ElMessage.success('登入成功')
