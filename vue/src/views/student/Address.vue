<template>
  <div>
    <div>
<!--      <el-card class="card">-->
<!--        <el-input style="margin-right: 5px; width: 240px" v-model="data.userId" placeholder="请输入搜索" prefix-icon="Search">-->
<!--        </el-input>-->
<!--        <el-button type="primary" plain @click="load()">查询</el-button>-->
<!--        <el-button type="primary" plain @click="reset()">重置</el-button>-->
<!--      </el-card>-->

      <el-card class="card" style="margin-top: 7px">
    ...
    <el-dialog v-model="data.formVisible" title="地址信息" width="500" destroy-on-close>
...
const load=()=>{
  request.get('/address/selectPage',{  //?pageNum=1&pageSize=10
    params:{
...
const add=()=>{
  data.form.userId=JSON.parse(localStorage.getItem('user')).id,
...
const del=(id)=>{
  ElMessageBox.confirm("删除数据后无法恢复，确认删除吗","确认删除",{type:"warning"}).then(()=>{
    request.delete('/address/deleteById',{
      params:{
        id:id
      }
    }).then(res=>{
...
const batchDelete=()=>{
  ...
  ElMessageBox.confirm("批量删除删除数据后无法恢复，确认删除吗",{type:"warning"}).then(()=>{
    request.delete('/address/deleteBatch',{data:data.ids}).then(res=>{
      if(res.code=='200'){
        ElMessage.success("操作成功")
        load() //新增后重新加载数据
      } else{
